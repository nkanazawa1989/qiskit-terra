...
def transpile(circuits: Union[QuantumCircuit, List[QuantumCircuit]],
              ...
              instruction_durations: Optional[List[Tuple[str, List[int], int]]] = None,
    ...
    Args:
        ...
        scheduling_method: Name of scheduling pass.
          ...
          qubit resource. alias: ``'asap'``)
            ...
            * ``'asap_timestep'``: Schedule instructions as early as possible with timesteps.
            * ``'alap_timestep'``: Schedule instructions as late as possible with timesteps.
...
def _transpile_circuit(circuit_config_tuple: Tuple[QuantumCircuit, Dict]) -> QuantumCircuit:
    ...
    if scheduling_method is not None:
        ...
        elif scheduling_method == 'asap' or scheduling_method == 'as_soon_as_possible':
            pass_manager.append(ASAPSchedule(instruction_durations))
        elif scheduling_method == 'alap_timestep':
            pass_manager.append(ALAPTimestepSchedule(instruction_durations))
        elif scheduling_method == 'asap_timestep':
            pass_manager.append(ASAPTimestepSchedule(instruction_durations))
        else:
